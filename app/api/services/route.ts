  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Erro ao buscar serviços: ${response.status} ${response.statusText}`, details: errorText },
...
export async function POST(request: Request) {
  try {
    ...
    console.log("POST request to external API:", `${API_BASE_URL}/api/service`);
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { 
          error: `Erro ao criar serviço: ${response.status} ${response.statusText}`, 
