  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: `Erro ao buscar produtos: ${response.status} ${response.statusText}`, details: errorText },
...
export async function POST(request: Request) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { 
          error: `Erro ao criar produto: ${response.status} ${response.statusText}`, 
    ...
    console.error("Error in product POST API route:", error);
    return NextResponse.json(
      { 
        error: "Erro ao conectar com a API",
        message: error instanceof Error ? error.message : "Erro desconhecido"
